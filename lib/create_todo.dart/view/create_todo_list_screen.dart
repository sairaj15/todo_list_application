  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CreateTodoBloc, CreateTodoState>(
      builder: (context, state){
        return PopScope(
          canPop: state.todoListStatus == CreateTodoStatus.loading ? false : true,
          child: Scaffold(
            appBar: AppBar(
              title: Text(
                widget.todoListModel == null ? 'Create Todo List' : 'Edit Todo List',
              ),
              centerTitle: true,
            ),
                ...
                if (formKey.currentState!.validate()) {
                  ...
                  if (widget.todoListModel != null) {
                    context.read<CreateTodoBloc>().add(
                      CreateTodoEventUpdateEvent(todoListModel: widget.todoListModel!.copyWith(
                        title: title,
                        description: description,
                      ))
                    );
                    context.read<CreateTodoBloc>().add(
                      CreateTodoEventSaveEvent(
                        title: title,
                        description: description,
                      )
                    );
